#!/usr/bin/perl
#
#  mtmd_forcing_verify.pl -- verification suite for the paper
#
#     "Mixed Triple Metric Dimension of Polyhedral Graphs:
#      Local Forcing and Extremal Families"
#
#  by R. Adawiyah, Rinurwati and M. Nagaraj.
#
#  This single program reproduces every row of Table 2 of that paper, in the
#  order in which the rows appear.  There are eleven checks and eleven rows.
#
...
#  mixed metric dimension mmd(G), and restricting it to F(G) gives the face
#  metric dimension.
#
#  The forcing criterion of the paper is
#
...
#  SCOPE
#  -----
#  The paper treats the triangular-face regime.  A companion paper treats the
#  complementary triangle-free regime, and its exact values for the prisms, the
#  stacked prisms C_n x P_r and the layered family L(n,r,sigma) are NOT checked
#  here.  The prism and stacked-prism builders are nevertheless retained,
#  because several checks below run over a corpus that includes them.  On those
#  graphs the forcing criterion must fire nowhere, and confirming that is part
#  of the test.
#
#  USAGE
#  -----
#      perl mtmd_forcing_verify.pl              # full run, ranges as in Table 2
#      perl mtmd_forcing_verify.pl --quick      # reduced ranges, ~10 seconds
#      perl mtmd_forcing_verify.pl --only=NAME  # run a single check
#      perl mtmd_forcing_verify.pl --list       # list check names
#
...
# forced set F(G) together with a per-pair audit of the forcing criterion
sub forced_set_audit {
    ...
    for my $f (@{ $G->{F} }) {
        ...
        for my $perm ([$x,$y,$z], [$y,$z,$x], [$z,$x,$y]) {
            ...
            # the forcing criterion: containment ==> unique separator (and it is w)
...
# ----------------------------------------- row 4 of Table 2  (bipyramid-theorem)
...
# ------------------------------------------- row 5 of Table 2  (forcing-theorem)
register('forcing-theorem',
  'the forcing criterion (containment ==> unique separator) across all test graphs',
  sub {
    my $hi = $QUICK ? 6 : 10;
...
# -------------------------------------------- row 6 of Table 2  (faces-complete)
...
# --------------------------------------------- row 7 of Table 2  (lower-bound-3)
...
# -------------------------------------------- row 10 of Table 2  (face-dimension)
